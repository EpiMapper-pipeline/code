...
def bin_count_macs2(peak_file,chromosome_sizes, blacklist_removed_bin):
    ...
    sort_cmd = "sort -k1,1V -k2,2n -k3,3n " + peak_file.replace(".bed", "_2cleaned.bed") + " > " +peak_file.replace(".bed","_sorted.bed")  
    ...
    return fold_file.replace(".bed", "_sorted.bed")
...
def combined_dfs(files):
    ...
    for i in range(1,len(all_df)):
        ...
        if i==1:
            combined_df=pd.merge(all_df[i-1],all_df[i],on=['chrom','starts','ends'], how='outer')
        else:
            combined_df=pd.merge(combined_df,all_df[i],on=['chrom','starts','ends'] , how='outer')
...
def make_combined_signal(peaks_files, chromosome_sizes,blacklist_removed_bin,peaks_folder, searchStr1,searchStr2, out_path):
    for file in peaks_files:
        bin_count_macs2(file,chromosome_sizes,blacklist_removed_bin)
...
def enrichment_main(peaks_folder,blacklist_removed_bin, chromosome_sizes, out_combined_files, searchStr1,searchStr2):
    ...
    peaks_files = glob.glob(os.path.join(peaks_folder,"*peaks*sorted*.bed"))
