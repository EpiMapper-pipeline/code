...
def macs2_run(macs2,peakCalling, bam_dir,control,percentage, g_size,macs2_control,macs2_top):
    ...
    if control:
        ...
        for sample in conditional:
            ...
            cmd_macs_top = "macs2 callpeak -t " +file +" -f  BAMPE -g "+ g_size+" -n " + name+"_macs2_top_"+percentage+" --outdir " + macs2_top
...
def macs2_summary(macs2,summary_tables, macs2_control, macs2_top):
    ...
    return sorted_files, peak_summary, peak_width
...
def seacr_summary(seacr, summary_tables,seacr_top,seacr_control):
    ...
    peak_summary = pd.DataFrame(columns = ["Sample","Replication", "peakType", "peakN" ])
    p_files = glob.glob(os.path.join(seacr_top, "*"))
    if os.path.exists(seacr_control):
       p_files = p_files + glob.glob(os.path.join(seacr_control,"*"))
...
def bedtools_seacr(sorted_files, seacr, peak_summary, fragments,sum_tbl, seacr_control, seacr_top):
...
def bedtools_macs2(sorted_files, macs2, peak_summary, fragments,sum_tbl, macs2_control, macs2_top):
...
def plot(fragInPeak_df,summary_tables, width,types):
    """
    Creates 4 plots 
    ...
    if len(types)>1:
        ...
        if fragInPeak_df.shape[0]>0: 
            ...
            s.add_legend()
            s.fig.subplots_adjust(hspace=0.9, wspace=.40)
            ...
            v.map_dataframe(sns.boxplot,x = "Sample", y = "Frips",dodge=False)
            ...
            l.fig.subplots_adjust(hspace=0.9, wspace=.40)
...
def peakcall_seacr(seacr_path,peakCalling, summary_tables,sum_tbl,bedgraph,control, fragments, percentage,skip_plot):
    ...
    if skip_plot:
        ...
        p_files = glob.glob(os.path.join(seacr_top,"*.bed"))
        if isinstance(control, str):
            p_files+= glob.glob(os.path.join(seacr_control,"*"))
        
        fragInPeak_df = bedtools_seacr(p_files, seacr, peak_summary, fragments,sum_tbl, seacr_control, seacr_top)
    
        plot(fragInPeak_df,summary_tables, width,types)
    
     
    
def peakcall_macs2(peakCalling, bam_dir,control,percentage,summary_tables, fragments,sum_tbl, skip_plot,genome_size):
    ...
    if skip_plot:
        print("Performing peak calling without calculating peak reproducibility or generating plots.")
        types =macs2_run(macs2,peakCalling, bam_dir,control,percentage,macs2_control,macs2_top)
        print("Done with calling peaks using MACS2 software.\nCalled peaks avalible at: "+ macs2)
        exit(0)
    else:
        types =macs2_run(macs2,peakCalling, bam_dir,control,percentage,genome_size, macs2_control, macs2_top)
        
        sorted_files, peak_summary, peak_width = macs2_summary(macs2,summary_tables, macs2_control,macs2_top)
        
        full_peak_summary=bedtools_macs2(sorted_files, macs2, peak_summary, fragments,sum_tbl,macs2_control, macs2_top)
       
        plot(full_peak_summary, summary_tables,peak_width, types)
...
def check_input(args,summary_tables):
    ...
    if software=="seacr": 
        ...
        if args.seacr_path is not None:
            seacr_path = args.seacr_path
            if not os.path.exists(seacr_path) or not os.path.isfile(seacr_path):
                print("Chosen path to SEACR shell script: "+ seacr_path +" does not exist or is not a file.\nPlease check your path.")
                exit(1)
        else:
            print("No SEACR shell script provided.\nPlease provide the path to SEACR shell script in the -s parameter")
            exit(1)
    ...
    return tbl, percentage, fragments, control, skip_plot, software
...
def run(args):
    ...
    tbl, percentage, fragments, control, skip_plot, software =check_input(args,summary_tables)
    ...
    if args.software == "macs2":
        ...
        peakcall_macs2(peakCalling, bam_dir,control,percentage,summary_tables, fragments,sum_tbl,skip_plot,genome_size)
    else:
